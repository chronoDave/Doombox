...
export const thumbSelector = (state: State) => (size: number) => (id: string | null) => {
  if (!id || !state.app.directory.thumbs) return 'icons/icon_light.png';
  return new URL(`${id}/${size}.jpg`, `${state.app.directory.thumbs}/`).href;
};
...
export const populateSongs = (state: State) => (songs: string[]) => songs
  .reduce<Array<Song & { image: string }>>((acc, cur) => {
    ...
    if (song) acc.push({ ...song, image: thumbSelector(state)(96)(song.image) });
    return acc;
  }, []);
...
export const populateAlbums = (state: State) => (albums: string[]) => albums
  .reduce<Array<Album & { image: string }>>((acc, cur) => {
    ...
    if (album) acc.push({ ...album, image: thumbSelector(state)(128)(album.image) });
    return acc;
  }, []);
